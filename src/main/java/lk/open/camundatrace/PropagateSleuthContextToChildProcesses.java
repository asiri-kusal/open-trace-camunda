...
import static lk.open.camundatrace.CamundaSlueuthContextInjectingAspect.X_SLEUTH_TRACE_DURATION;
import static lk.open.camundatrace.CamundaSlueuthContextInjectingAspect.X_SLEUTH_TRACE_ID;
import static lk.open.camundatrace.CamundaSlueuthContextInjectingAspect.extractSerializedContext;
import static lk.open.camundatrace.CamundaSlueuthContextInjectingAspect.restoreTracingContext;
import static lk.open.camundatrace.CamundaSlueuthContextInjectingAspect.serializeAndInjectTracingContext;
...
public class PropagateSleuthContextToChildProcesses {
     ...
     */
    public static Object propagateSleuthContextToChildProcesses(ProceedingJoinPoint pjp, Tracing tracing)
        throws Throwable {
        if (tracing == null) {
            return pjp.proceed();
        }
        if (!(pjp.getArgs()[0] instanceof ExecutionEntity)) {
            return pjp.proceed();
        }
        if (!(pjp.getArgs()[1] instanceof VariableMap)) {
            return pjp.proceed();
        }
        ...
        Map<String, String> sleuthTraceSerialized = extractSerializedContext(
            sourceVariables.get(X_SLEUTH_TRACE_CONTEXT));
