...
public class CamundaSlueuthContextInjectingAspect {
     ...
     */
    @Autowired
    public CamundaSlueuthContextInjectingAspect(RuntimeService runtimeService, Tracing tracing) {
        CamundaSlueuthContextInjectingAspect.tracing = tracing;
        CamundaSlueuthContextInjectingAspect.runtimeService = runtimeService;
    }
    ...
    @Around("execution(public void org.camunda.bpm.engine.ExternalTaskService.complete(String, String, java.util.Map))")
    public void javaDelegateSpan(ProceedingJoinPoint pjp) throws Throwable {
        JavaDelegateSpanDefinition.restExternalSpan(pjp, tracing, runtimeService);
    }
    ...
    public Object propagateTracingFromCamundaSerializedContextInAnyConnector(ProceedingJoinPoint pjp) throws Throwable {
        return ConnectorCallExecutionSpanDefinition.
                                                       propagateTracingFromCamundaSerializedContextInAnyConnector(pjp,
                                                                                                                  tracing);
    }
}
